...
RUN echo 'snsd' | vncpasswd -f > $HOME/.vnc/passwd
...
RUN echo 'whoami ' >>/snsd.sh

RUN echo 'cd ' >>/snsd.sh

RUN echo "su -l -c 'vncserver :2000 -geometry 1360x768' "  >>/snsd.sh

RUN echo 'cd /noVNC-1.2.0' >>/snsd.sh

RUN echo './utils/launch.sh  --vnc localhost:7900 --listen 8900 ' >>/snsd.sh

RUN chmod 755 /snsd.sh
